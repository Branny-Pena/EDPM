...
NodoArbol* crear_nodoab(ElementoArbol dato,ArbolB arbol_izq,ArbolB arbol_der){
    ...
    nuevo->elemento.llave = (char*)malloc(sizeof(char)*(strlen(dato.llave)+1));
    strcpy(nuevo->elemento.llave, dato.llave);
    nuevo->elemento.valor = (char*)malloc(sizeof(char)*(strlen(dato.valor)+1));
    strcpy(nuevo->elemento.valor, dato.valor);
...
void imprimirelemento(ElementoArbol dato){
    printf("[%s %s] ",dato.llave, dato.valor);    
}
